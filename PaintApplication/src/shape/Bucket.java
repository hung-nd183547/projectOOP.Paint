...
public class Bucket extends Shape implements DrawType{
    ...
    public void draw(BufferedImage image){
        color_before = image.getRGB(this.startPoint.x, this.startPoint.y);//tao mau ban dau cua diem anh
    ...
    public void spreadFill(BufferedImage image){
        ...
        if(color_before == color_fill)//check color tai diem xet neu da to dung mau thi dung
            ...
            image.setRGB(this.startPoint.x, this.startPoint.y, color_fill);//to cho diem anh toa do click
            listPoints.add(this.startPoint);//them click vao dau cua hang doi de thuc hien loang tu click
            while(!listPoints.isEmpty()){
                ...
                //pointEx.x,y ><= de xet xem co ra ngoai pham vi cua cua so hay khong->>>to mau nen backgroud khi khong co vien
